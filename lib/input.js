...
proto._readContents = function() {
  throw new Error('Not implement _readContents');
};
...
proto.read = proto._readContent = function () {
  ...
  if (la === undefined) {
    debug('[_readContent] lookahead: undefined');
    return;
  }

  debug('[_readContent] lookahead 0x' + la.toString(16));
...
proto._readObject = function () {
  ...
  var la = this.lookAhead();
  debug('[_readObject] lookahead: 0x' + la.toString(16));
  ...
  if (la === cons.TC_OBJECT) {
    ...
    throw new Error('[_readContent] Unkown type: 0x' + la.toString(16));
  }
};
...
proto._readNewObject = function () {
  ...
  debug('[_readNewObject] ----- Start -----');

  this.in.get();

  var obj = { $: {} };
  var $class = obj.$class = this._readClassDesc();
  var $fields = obj.$fields = concatFields($class);
  this._newHandle(obj);
  this._readClassData(obj);

  debug('[_readNewObject] ----- End -----\n got %j, %d fields', $class.name, $fields.length);
...
proto._readNewClassDesc = function () {
  ...
  var la = this.lookAhead();
  debug('[_readNewClassDesc] Got lookahead: 0x' + la.toString(16));
  ...
  if (la === cons.TC_CLASSDESC) {
    ...
    this._readClassDescInfo(obj);

    debug('[_readNewClassDesc] Got class: %j', obj);
    return obj;
    ...
    throw new Error('[_readNewClassDesc] Unkown type: 0x' + la.toString(16));
  }
};
...
proto._readClassDesc = function () {
  ...
  var la = this.lookAhead();
  debug('[_readClassDesc] Got lookahead: 0x' + la.toString(16));
  ...
  if (la === cons.TC_CLASSDESC) {
    ...
    throw new Error('[_readClassDesc] Unknown type: 0x' + la.toString(16));
  }
};
...
proto._readFields = function () {
  ...
  debug('[_readFields] Got %d fields: %j', count, fields);
  return fields;
};
...
proto._readSuperClassDesc = function () {
  // superClassDesc:
  //   classDesc
  debug('[_readSuperClassDesc] ----- Start -----');
  var superClass = this._readClassDesc();
  debug('[_readSuperClassDesc] ----- End -----, got %j', superClass);
  return superClass;
};

proto._readClassDescInfo = function (obj) {
  // classDescInfo:
...
proto._readFieldDesc = function () {
  ...
  if (type === '[' || type === 'L') {
    field.classname = this._readObject();
  }
...
proto._readUTFString = function (isLong) {
  ...
  debug('[_readUTFString] Got: %s (length: %d)', str, len);
  return str;
};
