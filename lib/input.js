...
proto.read = proto.readObject = proto._readContent = function () {
  try {
    ...
    err.name = 'InvaildStreamContentError';
    err.data = this.in._bytes.toString('base64');
    throw err;
  }
}
...
proto._readClassData = function (obj) {
  ...
  if (flags & cons.SC_SERIALIZABLE) {
    ...
    if(!customObject && classname.indexOf('java.lang.') == -1) {
      classname = 'java.util.HashMap';
      customObject = objects[classname];
    }
    ...
    if (flags & cons.SC_WRITE_METHOD) {
      ...
      if (hasReadObjectMethod) {
        ...
        throw new Error('customObject.readObject() not implemented for Java class "'+ classname + '".');
      }
    } else {
...
proto.readBlockHeader = function() {
  ...
  if (flag === cons.TC_BLOCKDATA) {
    ...
    return size;
  }else if (flag == cons.TC_ENDBLOCKDATA) {
    return null;
  } else {
