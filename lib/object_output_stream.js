...
// return;
  ...
  } else if (obj instanceof types.JavaObjectArray) {
    if (obj.value.length === 0) {
      this.writeNull();
    } else {
      desc = ObjectStreamClass.lookup(obj, true);
      this.writeArray(obj.value, desc, unshared);
    }
 ...
 */
proto.defaultWriteFields = function (obj, desc) {
// Class<?> cl = desc.forClass();
...
HandleTable.prototype.assign = function (obj) {
  if (this.refs.indexOf(obj) < 0) {
    this.refs.push(obj);
  }
};

HandleTable.prototype.lookup = function (obj) {
