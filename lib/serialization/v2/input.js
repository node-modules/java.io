...
proto.read = proto._readContent = function (withType) {
  // contents:
  //   content
  //   contents content
  ...
  if (type === cons.TC_BLOCKDATA) {
    return this._readBlockDataShort(withType);
  }

  if (type === cons.TC_BLOCKDATALONG) {
    return this._readBlockDataLong(withType);
  }

  return this._readObject(withType);
};
...
proto._readNewObject = function (withType) {
  ...
  if (classDesc.name.indexOf('java.lang.') === 0
    && obj.$fields.length === 1
...
proto._readNewClassDesc = function () {
  ...
  var desc = {
    ...
    serialVersionUID: this.readLong().toString(), // serialVersionUID
