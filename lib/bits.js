 ...
 */
function toLong(v) {
  if (v instanceof Long || (v.hasOwnProperty('high') && v.hasOwnProperty('low'))) {
    return v;
  }
  ...
  throw new TypeError(v + ' can not convert to long');
}

function toLongBytes(v, b) {
  b = b || new Buffer(8);
  v = toLong(v);
  b.writeInt32BE(v.high, 0);
  b.writeInt32BE(v.low, 4);
  return b;
}
