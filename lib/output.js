...
proto._writeSerialData = function(obj) {
  ...
  if (flags & cons.SC_SERIALIZABLE) {
    if (flags & cons.SC_WRITE_METHOD) {
      if (className in objects && 'writeObject' in objects[className]) {
        objects[className].writeObject(this, obj);
        out.put(cons.TC_ENDBLOCKDATA);
      } else {
        throw new Error('Class "'+ name + '" dose not be added in or not implement writeObject()');
      }
...
proto.writeBlockHeader = function (len) {
  this.out.put(new Buffer([0x77, len]));
}
